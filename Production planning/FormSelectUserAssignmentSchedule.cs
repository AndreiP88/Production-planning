...
{
    public partial class FormSelectUserAssignmentSchedule : MaterialForm
    {
        ...
        public FormSelectUserAssignmentSchedule(int userId)
        {
            InitializeComponent();
        ...
        private async Task LoadHistory()
        {
            ...
            try
            {
                List<EmployeeScheduleHistoryRow> schedHistory = await employeeService.GetEmployeeScheduleHistoryAsync((ulong)_userId);

                int index = 0;

                foreach (EmployeeScheduleHistoryRow schedule in schedHistory)
                {
                    index++;
