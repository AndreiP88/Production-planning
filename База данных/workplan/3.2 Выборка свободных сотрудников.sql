...
WITH EmployeeStatus AS (
    SELECT 
        ...
        (SELECT eq_n.name FROM employee_equipment_assignments eqa_n
         JOIN equipment eq_n ON eqa_n.equipment_id = eq_n.id
         WHERE eqa_n.employee_id = e.id AND eqa_n.valid_from <= @target_date 
         ORDER BY eqa_n.valid_from DESC LIMIT 1) AS default_eq_name,
         
        -- 4. Проверка отсутствия
        EXISTS (SELECT 1 FROM absences abs 
                WHERE abs.employee_id = e.id AND @target_date BETWEEN abs.start_date AND COALESCE(abs.end_date, '2099-12-31')) AS is_absent,
                
        -- 5. Подмена (override) - берем ID и описание
        (SELECT ovr.id FROM schedule_overrides ovr 
         WHERE ovr.employee_id = e.id AND ovr.override_date = @target_date AND ovr.status = 2 LIMIT 1) AS override_id,
        ...
        (SELECT CONCAT(eq_ovr.name, ' (смена ', sd_ovr.shift_number, ')') 
         ...
         WHERE ovr.employee_id = e.id AND ovr.override_date = @target_date AND ovr.status = 2 LIMIT 1) AS current_manual_work
    FROM employees e
...
SELECT 
    ...
    -- ТЕКУЩИЙ СТАТУС
    CASE 
        WHEN es.is_absent THEN '❌ ОТСУТСТВУЕТ'
        WHEN es.current_manual_work IS NOT NULL THEN CONCAT('📌 ПОДМЕНЯЕТ: ', es.current_manual_work)
        WHEN es.plan_shift_num = @target_shift_num AND (
            EXISTS (SELECT 1 FROM equipment_daily_plan edp 
                    WHERE edp.equipment_id = es.default_eq_id 
                      AND edp.plan_date = @target_date 
                      AND edp.is_cancelled = 1)
            OR
            ( (SELECT COALESCE((SELECT staffing_mode FROM equipment_staffing_history WHERE equipment_id = es.default_eq_id AND valid_from <= @target_date ORDER BY valid_from DESC LIMIT 1), eq_orig.staffing_mode)
               FROM equipment eq_orig WHERE eq_orig.id = es.default_eq_id) = 'manual_only'
               AND NOT EXISTS (SELECT 1 FROM equipment_daily_plan edp2 WHERE edp2.equipment_id = es.default_eq_id AND edp2.plan_date = @target_date) )
        ) THEN '🟢 СВОБОДЕН (Простой станка)'
        WHEN es.plan_shift_num = @target_shift_num THEN '⚙️ ЗАНЯТ (Свой станок)'
        ELSE '---'
    END AS "Статус",
    
    -- ИНДЕКС НАЗНАЧЕНИЯ (из таблицы overrides)
    es.override_id AS "ID_Override",
    
    -- ПРИОРИТЕТЫ
    CASE 
        WHEN es.is_absent OR es.current_manual_work LIKE CONCAT('%(смена ', @target_shift_num, ')%') THEN 5 
        WHEN es.plan_shift_num = @target_shift_num AND (
