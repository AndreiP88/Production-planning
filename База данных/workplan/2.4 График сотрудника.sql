...
-- 3. ИТОГОВЫЙ ВЫВОД С ПРИОРИТЕТОМ РУЧНЫХ ПРАВОК
SELECT 
    f_date AS "Дата", 
    f_shift AS "Смена", 
    ...
    f_type AS "Тип"
FROM (
    -- КАТЕГОРИЯ А: Ручные назначения (даже если есть больничный)
    SELECT 
    ...
    -- КАТЕГОРИЯ Б: Дни с отсутствием (только те дни, где НЕТ ручных назначений вообще)
    SELECT 
    ...
    -- Проверка: в этот день не должно быть НИ ОДНОЙ ручной правки
    WHERE NOT EXISTS (SELECT 1 FROM schedule_overrides ov WHERE ov.employee_id = @target_emp_id AND ov.override_date = c.day_date)

    UNION ALL

    -- КАТЕГОРИЯ В: Обычный план (если нет ни отсутствия, ни ручных правок)
    SELECT 
        sa.day_date AS f_date,
        sa.shift_name AS f_shift,
