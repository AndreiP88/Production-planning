...
WITH EmployeeStatus AS (
    SELECT 
        ...
        -- 4. Причина отсутствия (подробная)
        ...
        -- 5. Флаг явной отмены смены в оверрайдах
...
SELECT 
    ...
    CASE 
        ...
        -- Добавлена логика: если смена отменена флагом
        WHEN es.is_plan_cancelled THEN '🟢 СВОБОДЕН (Смена отменена)'
        -- Логика простоя станка
    ...
    -- ПРИОРИТЕТЫ
    CASE 
        WHEN es.abs_reason IS NOT NULL OR es.current_manual_work LIKE CONCAT('%(смена ', @target_shift_num, ')%') THEN 5 
        -- Свободные: отмена смены или простой станка
        WHEN es.is_plan_cancelled OR (es.plan_shift_num = @target_shift_num AND (
