...
SELECT 
    ...
    CASE 
        ...
        WHEN (
            ...
            AND (
                -- Работа приоритетнее болезни: считаем либо оверрайд, либо план без болезни
    ...
    GROUP_CONCAT(DISTINCT 
        CASE WHEN esl.l_type = 'PLAN' AND esl.eq_id = qs.eq_id AND esl.shift_id = qs.shift_id THEN
            CONCAT(esl.full_name, 
                CASE 
                    WHEN EXISTS (SELECT 1 FROM schedule_overrides o_m WHERE o_m.employee_id = esl.emp_id AND o_m.override_date = qs.day_date AND o_m.shift_id = qs.shift_id AND o_m.status = 2 AND o_m.is_cancellation = 0)
                        THEN (CASE WHEN (SELECT equipment_id FROM schedule_overrides WHERE employee_id = esl.emp_id AND override_date = qs.day_date AND shift_id = qs.shift_id AND status = 2 LIMIT 1) != qs.eq_id 
    ...
    -- УТВЕРЖДЕННЫЙ ФАКТ (ПРИОРИТЕТ: OVR > ABSENCE > PLAN)
    GROUP_CONCAT(DISTINCT 
        CASE 
            WHEN (
                -- 1. Сюда назначен утвержденный оверрайд (приоритет №1, игнорирует больничный)
                (esl.l_type = 'OVR' AND esl.l_status = 2 AND esl.eq_id = qs.eq_id AND esl.shift_id = qs.shift_id AND esl.is_cancellation = 0)
                OR
                -- 2. План (если нет больничного И нет утвержденной правки на эту смену)
                (esl.l_type = 'PLAN' AND esl.eq_id = qs.eq_id AND esl.shift_id = qs.shift_id 
                 AND NOT EXISTS (SELECT 1 FROM absences a WHERE a.employee_id = esl.emp_id AND qs.day_date BETWEEN a.start_date AND COALESCE(a.end_date, '2099-12-31'))
