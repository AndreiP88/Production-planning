...
SELECT 
    ...
    CASE 
        ...
        WHEN (
            ...
            AND (
                NOT EXISTS (SELECT 1 FROM absences a WHERE a.employee_id = el.emp_id AND qs.day_date BETWEEN a.start_date AND COALESCE(a.end_date, '2099-12-31'))
                OR EXISTS (SELECT 1 FROM schedule_overrides o_f WHERE o_f.employee_id = el.emp_id AND o_f.override_date = qs.day_date AND o_f.shift_id = qs.shift_id AND o_f.status = 2 AND o_f.is_cancellation = 0)
            )
            AND (
    ...
    -- ПЛАН_ГРАФИК (Причина отсутствия всегда видна, если она есть)
    ...
    -- НАЗНАЧЕНИЯ (Сюда попадет человек, выведенный с больничного)
    GROUP_CONCAT(DISTINCT 
    ...
    -- УТВЕРЖДЕННЫЙ ФАКТ (Оверрайд игнорирует больничный и попадает сюда)
    GROUP_CONCAT(DISTINCT 
        CASE 
            WHEN (
                -- Либо это утвержденное ручное назначение (игнорирует болезнь)
                (esl.l_type = 'OVR' AND esl.l_status = 2 AND esl.eq_id = qs.eq_id AND esl.shift_id = qs.shift_id AND esl.is_cancellation = 0)
                OR
                -- Либо это план без болезни И без каких-либо отменяющих правок
